...
func main() {
	 ...
	 */
	 //1.路径
	 fileName1:="/Users/didi/Desktop/Recent-Notes/Go入门笔记/第一阶段源码/14_IO操作/l_file/ab.txt"
	 fileName2:="bb.txt"
	 ...
	 //3.创建文件:Create采用模式0666（任何人都可读写，不可执行）创建一个名为name的文件，如果文件已存在会截断它（创建一个空文件）
	 file1,err :=os.Create("/Users/didi/Desktop/Recent-Notes/Go入门笔记/第一阶段源码/14_IO操作/l_file/cc.txt")
	 if err != nil{
	 	fmt.Println("err：",err)
	 	return
	 }
	...
	/*
	第一个参数：文件名称
	第二个参数：文件的打开方式，前面三个任选其一，后边可以多选，中间用‘｜’隔开
		const (
	 ...
	 */
	 // os.O_RDONLY 0x0
	 // os.O_WRONLY 0x1
	//  file4,err := os.OpenFile(fileName1,os.O_RDONLY,os.ModePerm)
	//  if err != nil{
	//  	fmt.Println("err:",err)
	//  	return
	//  }
	...
	//fmt.Println("删除文件成功。。")
	//删除目录
	// err :=  os.RemoveAll("/Users/didi/Desktop/Recent-Notes/Go入门笔记/第一阶段源码/14_IO操作/l_file/ab.txt")
	// if err != nil{
	// 	fmt.Println("err:",err)
	// 	return
	// }
